...
class DictShim(dict):
	def __getattribute__(self,n):
		if n.startswith('__'): return dict.__getattr__(self,n)
		return transform(self[n])#
		
	# def __getitem__(self,*n):
		# return transform(dict.__getitem__(self,*n))
...
class ListShim(list):
	...
	def __getslice__(self,*a):
		return transform(list.__getslice__(self,*a))
...
class NameAccumulator(object):
	...
	def notransform(self,**kw):
		return call_api(self.name[1:],kw,self.token)
		
class VK(object):
	def __init__(self,user,pwd,clid,scope):
		'VK(user,pwd,appid,scope)'
...
def vklogin(scope=[],appid=3463155):
	import getpass
	u=raw_input("Username: ")
	p=getpass.getpass()
	return VK(u,p,appid,scope)